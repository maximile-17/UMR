...
int main(int argc, char **argv)
{
	...
	for(imr=0; imr<mr_num; imr++){
		emr[imr] = ibv_reg_mr(pd, (void *)((uint64_t)(uintptr_t)buf_umr + imr * parameters.stride), parameters.stride, IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE);
		if (!emr[imr]) {
...
//		qp_attr.ah_attr.grh.flow_label = 0;
	...
	for (int test = SGRS; test < TEST_END; test++) {
		...
		for(i=0; i<mr_num; i++){
			...
			mem_reg_list[i].length = parameters.block_size;
